...
if [[ ! -d reclab/__check_build ]]; then
    echo "This must be run from the reclab project directory"
    exit 3
fi
...
if [[ "${CIRCLE_BRANCH}" =~ ^master$|^[0-9]+\.[0-9]+\.X$ ]]; then
    ...
    git remote add origin https://${GH_NAME}:${GH_TOKEN}@github.com/${GH_NAME}/reclab.git
