...
from reclab.model_selection import train_test_split, check_cv, KFold
...
def test_check_cv():
    ...
    assert not cv2.shuffle  # important since True is the default
...
def test_kfold_cv():
    bunch = {
        ...
        'products': np.array([5, 1, 8, 9, 2, 0, 7, 6, 6, 1, 3, 5,
                              1, 6, 7, 1, 0, 0, 6, 8, 6, 0, 4, 9,
                              6, 1, 7, 2, 3, 5, 9, 8, 7, 3, 7, 8,
                              7, 6, 2, 6, 2, 1, 4, 3, 8, 7, 4, 4,
                              3, 8, 0, 4, 7, 8, 4, 0, 2, 9, 6, 4,
                              7, 4, 3, 9, 5, 2, 1, 8, 2, 3, 8, 8,
                              7, 9, 6, 7, 1, 8, 7, 9, 1, 4, 3, 7,
                              2, 0, 5, 9, 4, 7, 3, 6, 4, 7, 2, 1,
                              9, 3, 5, 7]),
    ...
    n_expected = [(75, 25)] * 4
    actual_stored = [(x[0].nnz, x[1].nnz) for x in splits]
    assert n_expected == actual_stored, (n_expected, actual_stored)
